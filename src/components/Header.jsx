...
const Header = () => {
  ...
  return (
    <motion.div className='flex flex-col justify-center items-center text-center my-20'
      ...
      initial={{opacity:0,y:20}}
      animate={{opacity:1,y:0}}
      transition={{delay:0.6,duration:0.8}}>Generate stunning AI-powered images from text prompts instantly. Just describe your vision, and let our AI bring it to life!"</motion.p>
      ...
      initial={{opacity:0}}
      animate={{opacity:1}}
      transition={{default:{duration:0.5},opacity:{delay:0.8,duration:1}}}>Generate Images <img className="h-6" src={assets.star_group} alt="" /></motion.button>
      ...
      transition={{delay:1,duration:1}}>
        {Array(6).fill('').map((item,index)=>(
          ...
          whileHover={{duration:0.1,scale:1.05}} src={index % 2 === 0 ? assets.sample_img_2 : assets.sample_img_1} alt="" key={index} width={70}/>
        ))}
      </motion.div>
        ...
        initial={{opacity:0}}
      animate={{opacity:1}}
      transition={{delay:1.2,duration:0.8}}>Generated images from Pictora</motion.p>
    </motion.div>
  )
}
